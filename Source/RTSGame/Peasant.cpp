...
void APeasant::BeginPlay()
{
  ...
  {
    ResourceMineTimes[ ResourceTypes[i].Type ] =
      ResourceTypes[i].Time;
  }
...
AGameObject* APeasant::PlaceBuildingAtRandomLocation( Types type )
{
  ...
  UE_LOG( LogTemp, Warning, TEXT("Placement failed") );
  return building;
}
...
void APeasant::Build( float t )
{
  if( building )
  {
    ...
    if( building->buildProgress > building->UnitsData.BuildTime )
    {
      ...
      building = 0;
      UE_LOG( LogTemp, Warning, TEXT("Building complete") );
    }
  }
}
...
void APeasant::Mine( float t )
{
  ...
  if( mining )
  {
    ...
    if( outsideDistance( attackTarget )   <=   MiningDistance )
    {
      ...
      if( MiningTime < 0 )
      {
        ...
        mining->Amount--;
        // Reset mining time
        MiningTime = ResourceMineTimes[ attackTarget->UnitsData.Type ];
        if( !mining->Amount )
        {
          mining->Destroy();
          attackTarget = 0;
