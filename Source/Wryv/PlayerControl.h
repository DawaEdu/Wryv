...
public:
  ...
  map<float,AGameObject*> PickNearest( FVector pos, FCollisionShape shape, set<Types> AcceptedTypes, set<Types> NotTypes );

  vector<AGameObject*> Pick( FVector pos, FCollisionShape shape );

  // Checks if the gameobject collides with any type listed. Used for pathfinder construction.
  vector<AGameObject*> Pick( AGameObject* object, UPrimitiveComponent* up );
  vector<AGameObject*> PickExcept( AGameObject* object, UPrimitiveComponent* up, vector<AGameObject*> except );

  // Gets all objects that are traced thru by vector
  vector<AGameObject*> Pick( const FVector2D& ScreenPosition );
  vector<AGameObject*> Pick( const FVector& eye, const FVector& lookDir );
  vector<AGameObject*> Pick( const FBox2DU& box, set<Types> AcceptedTypes, set<Types> NotTypes );
